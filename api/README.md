...
Production deployments authenticate with Vercel's automatically managed OIDC
token, so a DeepSeek API key is not required.
...
`feature:task-draft` tag for usage attribution. The daily request limit is
intentionally lightweight and stored only in the current warm Vercel instance;
it can reset during deployments, cold starts, or scaling.
