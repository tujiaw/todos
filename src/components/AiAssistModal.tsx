...
export const AiAssistModal: React.FC<AiAssistModalProps> = ({
  ...
  const showPromptChips = messages.length === 0 && !isLoading;
  ...
  return (
    <div className="fixed inset-0 z-[80] overflow-hidden">
      ...
      >
        ...
        >
          {showPromptChips && (
            <div className="flex flex-col items-center justify-center min-h-[42vh] text-center px-1 py-6">
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-50 to-indigo-100 dark:from-indigo-950/50 dark:to-indigo-950/70 text-indigo-600 dark:text-indigo-300 flex items-center justify-center shadow-inner mb-3">
              ...
              <p className="mt-1 text-[12px] text-slate-500 dark:text-slate-400 leading-relaxed max-w-[17rem]">
                Tap a prompt to run it, or type below. Create task fills a stub so you can finish the
                details.
              </p>
              <SuggestionChips
                    ...
                    >
                      {message.createdTasks.map((task, index) => (
                        <div
                          ...
                          className="rounded-xl border border-emerald-100 dark:border-emerald-900/40 bg-emerald-50/80 dark:bg-emerald-950/30 px-2.5 py-1.5 hover:border-emerald-300 dark:hover:border-emerald-700 transition-colors"
                        >
        ...
        <div className="px-4 sm:px-5 pt-3 pb-[calc(1rem+env(safe-area-inset-bottom,0px))] sm:pb-4 space-y-2 border-t border-slate-200/70 dark:border-slate-800/80 bg-white/80 dark:bg-slate-900/40">
          <p className="text-[10px] text-slate-400 dark:text-slate-500 px-0.5">
            Enter to send · Shift+Enter for new line
            {isLoading ? ' · Esc to stop' : ''}
          </p>
